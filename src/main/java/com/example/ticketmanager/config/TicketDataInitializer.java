...
public class TicketDataInitializer {
	...
	CommandLineRunner seedTickets(TicketRepository ticketRepository, UserRepository userRepository) {
		return args -> {
			...
			AppUser julien = userRepository.findByUsername("julien").orElseThrow();
			AppUser alice = userRepository.findByUsername("alice").orElseThrow();
			AppUser bob = userRepository.findByUsername("bob").orElseThrow();
			ticketRepository.saveAll(List.of(
					new Ticket("SecurityMigrationExecutor and SystemIndexMigrationExecutor should not be persistent tasks",
							"elastic/elasticsearch", "https://github.com/elastic/elasticsearch/issues/146662", TicketStatus.OPEN, julien),
					new Ticket("Taking a snapshot of an index closed prior to 7.2.0 fails with a NPE in 7.11.2",
							"elastic/elasticsearch", "https://github.com/elastic/elasticsearch/issues/70676", TicketStatus.OPEN, alice),
					new Ticket("ESQL should support a humanize option to quickly render values in human readable way.",
							"elastic/elasticsearch", "https://github.com/elastic/elasticsearch/issues/136533", TicketStatus.OPEN, bob),
					new Ticket("Add search in playlists", "TeamNewPipe/NewPipe",
							"https://github.com/TeamNewPipe/NewPipe/issues/3037", TicketStatus.OPEN, julien),
					new Ticket("See comment title \"edited\"", "TeamNewPipe/NewPipe",
							"https://github.com/TeamNewPipe/NewPipe/issues/13209", TicketStatus.OPEN, alice),
					new Ticket("Keep history control buttons visible when scrolling up long history lists",
							"TeamNewPipe/NewPipe", "https://github.com/TeamNewPipe/NewPipe/issues/13487", TicketStatus.OPEN, bob),
					new Ticket("[Task] Some of the sample guidance conforms to business usage", "apache/dubbo",
							"https://github.com/apache/dubbo/issues/13859", TicketStatus.OPEN, julien),
					new Ticket("[BUG] Questions about Dubbo Adaptive Load Balance", "apache/dubbo",
							"https://github.com/apache/dubbo/issues/15810", TicketStatus.OPEN, alice),
					new Ticket("[Task] All samples are entirely switched to SpringBoot3", "apache/dubbo",
							"https://github.com/apache/dubbo/issues/13862", TicketStatus.OPEN, bob),
					new Ticket("kc_locale parameter not working as expected in keycloak 25 (keycloak.v3 account theme)",
							"keycloak/keycloak", "https://github.com/keycloak/keycloak/issues/36116", TicketStatus.OPEN, julien)));
		};
	}
}
